...
func ShowWaitingList(w http.ResponseWriter, r *http.Request) {
	...
	coll := client.Database("orphanage").Collection("waitinglist") // Connect to collection
	// Write all data to cursor
	cursor, err := coll.Find(context.Background(), bson.D{})
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		glog.Fatal(err)
	}
	// Decoding each data and write to "result"
	...
	if err = cursor.Close(context.Background()); err != nil { // Close cursor
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
