...
func EditAdmin(w http.ResponseWriter, r *http.Request) {
	...
	updateFields := bson.D{} // Implement empty bson.D
	// Checking if not empty then add to "updateFields"
	if adminData.Phone != "" {
	...
	var result interface{} // Implement "result" for response
	// Checking if "updateFields" is not empty then add update operator "$set"
	var update bson.D
	...
	coll := client.Database("orphanage").Collection("admins") // Collection connect
	// Updating data
	updated, err := coll.UpdateOne(context.Background(), filter, update)
	if err != nil {
		glog.Fatal(err)
	}
	// Check if not found data in collection
